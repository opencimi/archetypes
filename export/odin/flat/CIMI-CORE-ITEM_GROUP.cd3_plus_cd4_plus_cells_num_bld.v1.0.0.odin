(P_AUTHORED_ARCHETYPE) <
	...
	description = <
		...
		>
		conversion_details = <
			["source_model"] = <"<http://s.details.loinc.org/LOINC/24467-3.html>">
		...
		details = <
			["en"] = <
				...
				purpose = <"Archetype for CIMI 'CD3+CD4+ (T4 helper) cells [#/volume] in Blood' concept">
				use = <"Use for CD3+CD4+ (T4 helper) cells [#/volume] in Blood.">
				keywords = <"CD3+CD4+ Cells", "T4 helper/inducer cells", "CD4+ T cells", "CD3 Cells", "Cell", "Cellularity", "Leu-4", "Leu4", "T3", "#", "Number concentration", "Count/volume", "Count", "ABS", "CT", "absolute", "absolutes", "Cnt", "Point in time", "Random", "Blood", "WB", "Whole blood", "Quantitative", "QNT", "Quant", "Quan", "Leu-3", "T-4", "Leu3", "W3/25", "Helper T cells", "Inducer T cells", "L3T4", "CELL MARKERS">
			>
		>
	...
	archetype_id = <
		physical_id = <"CIMI-CORE-ITEM_GROUP.cd3_plus_cd4_plus_cells_num_bld.v1.0.0">
		rm_publisher = <"CIMI">
		rm_package = <"CORE">
		rm_class = <"ITEM_GROUP">
		concept_id = <"cd3_plus_cd4_plus_cells_num_bld">
	...
	terminology = <
		...
		term_bindings = <
			...
			>
			["loinc"] = <
				["id1.1.1.1.1.1"] = <"http://loinc.org/id/24467-3">
				["at0.0.0.0.0.1"] = <"http://loinc.org/id/24467-3">
			>
		>
	>
	adl_version = <"2.0.5">
